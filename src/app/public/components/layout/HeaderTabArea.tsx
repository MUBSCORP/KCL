...
// ✅ 탭 정의: 라벨 + 이동 경로
const TABS = [
  { label: '대시보드', href: '/' }, // 현재 대시보드 경로
  { label: '실시간 모니터링', href: undefined },                // 루트 페이지
  { label: '이벤트 로그 상세', href: '/public/event-log' }, // 이벤트 로그 상세
  { label: 'Setting', href: undefined },          // 추후 필요 시 추가
];
...
function pathToIndex(pathname: string): number {
  ...
  if (pathname.startsWith('/public/dashboard-pack') || pathname.startsWith('/public/dashboard-cell')) return 0;
  return 0; // 기본은 대시보드
}
